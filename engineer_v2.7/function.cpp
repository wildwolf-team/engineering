...
int Function::lightAndLight(Mat& tempframe,Mat& currentframe,Mat& previousframe,Mat& frame,int framenum)
{
        ...
        {
            // 串口发送数据
            if((re_re[0].x+re_re[0].br().x)/2 < 640)
            SerialPort::RMserialWrite((re_re[0].x+re_re[0].br().x)/2,0,0,0);
            else if((re_re[0].x+re_re[0].br().x)/2 > 640)
