...
int toInference::resize_uniform(cv::Mat& src, cv::Mat& dst, cv::Size dst_size,object_rect& effect_area)
{
    int w = src.cols;
    int h = src.rows;
...
void toInference::draw_bboxes(const cv::Mat& bgr, const std::vector<BoxInfo>& bboxes,object_rect& effect_roi,vector<Rect>& rects)
{
    static const char* class_names[] = { "mineral"
    };
    ...
    for (size_t i = 0; i < bboxes.size(); i++)
    {
        ...
        if(bbox.score>0.65)
        {
            ...
            rects.push_back(cv::Rect(cv::Point((bbox.x1 - effect_roi.x) * width_ratio, (bbox.y1 - effect_roi.y) * height_ratio),
